...
import { registerProfileRoutes } from './modules/quit-profile/profile.routes';
import type { ProfileRepository } from './modules/quit-profile/profile.service';
...
export async function buildApp(options: BuildAppOptions = {}): Promise<FastifyInstance> {
  const app = Fastify({ logger: { redact: { paths: ['req.headers.authorization', 'req.headers.cookie', 'request.headers.authorization', 'request.headers.cookie', 'req.body', 'request.body'], censor: '[REDACTED]' } } });
  ...
  await registerProfileRoutes(app, options.profileRepository);
  return app;
}
